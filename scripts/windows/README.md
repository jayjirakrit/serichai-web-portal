...
Runs the whole portal behind **nginx on one public port** (default `8080`): nginx serves the built Angular app and reverse-proxies `/accounts/*` and `/health` to FastAPI (uvicorn), which listens on loopback only (`127.0.0.1:8000`, not reachable from the network) — the same shape as the Docker gateway. `setup.ps1` downloads the official nginx Windows zip into `scripts/windows/nginx/` (gitignored); its config is generated from `nginx.conf.template` on every start. Ports come from `portal.config.json` (`port`, `backendPort`). A Scheduled Task starts it at every boot as `SYSTEM`, so no one needs to log in.
...
|---|---|---|
| 1 | `setup.ps1` | Installs Python 3.13 + Node LTS (winget), backend venv + `pip install`, `npm ci` + build, downloads nginx, registers the `SerichaiWebPortal` startup task, starts it and waits for `/health`. Re-run to deploy updates. Flags: `-Port 8080`, `-SkipInstall`, `-SkipBuild`. |
| 2 | `open-firewall.ps1` | Inbound TCP allow rule for the port, **local subnet only**, Private/Domain profiles (never Public). Add `-MakeNetworkPrivate` if your Wi-Fi is classified "Public" (Windows default for new networks) to switch it to Private. `-Remove` deletes the rule. Prints the URLs other devices should use. |
...
| - | `uninstall.ps1` | Removes the startup task and firewall rule, stops nginx/uvicorn (keeps Python/Node/repo/`nginx/` folder). |
| - | `start-portal.ps1` | What the task runs (starts nginx + uvicorn). Can be run by hand for debugging. |

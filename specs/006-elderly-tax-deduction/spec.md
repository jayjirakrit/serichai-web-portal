...
2. **Given** a roster where more employees pass the age+salary test than the 10% headcount cap allows, **When** the calculation runs, **Then** only the highest-salary employees up to the cap show non-zero monthly amounts, while the rest show as eligible but excluded (all months zero).
...
2. **Given** an employee who is eligible by age and salary but excluded by the 10% cap, **When** the user views that employee's row, **Then** it is visibly distinguishable from a selected employee (e.g., marked "eligible, not selected" with zeroed months) so the exclusion is self-explanatory.
...
- An employee is over 60 by age but their average monthly net salary exceeds 15,000 THB — they are not eligible at all (both conditions must hold), and must not consume any of the 10% headcount cap.

## Requirements *(mandatory)*

### Functional Requirements

- **FR-001**: The tool MUST accept an uploaded payroll workbook matching the known template (roster sheet with per-employee identity, base wage, date of birth, and 12 months of base wage/overtime/bonus-where-applicable figures).
...
- **FR-003**: The tool MUST determine each employee's average monthly net salary (base wage + overtime, excluding bonus payments) as of the reference date, averaged only over the months for which that employee has data.
- **FR-004**: The tool MUST mark an employee eligible for the deduction only when both: age (as of the reference date) is over 60, AND average monthly net salary is at or below the deduction cap per employee per month.
- **FR-005**: Among eligible employees, the tool MUST rank them by average monthly net salary (highest first), breaking exact ties by the employee's original sequence number, and select only the top-ranked employees up to a limit of 10% of the total employee headcount (rounded down); eligible employees beyond that limit remain visible in the results but are marked as not selected.
- **FR-006**: For each selected employee and each month, the tool MUST report that month's deductible amount as (that month's base wage + overtime) when it is at or below the deduction cap per employee per month, and as zero when it exceeds that cap — a single over-cap month never reduces the amount to the cap value, it zeroes that month only, without affecting other months or the employee's selected status.
...
- **Employee record**: One row of the payroll roster — sequence number, ID card number, name, base wage rate, date of birth, and, for each of the 12 months, base wage paid, overtime paid, and (for the two bonus months) bonus paid. Represents the raw input for one person.
- **Calculation result (per employee)**: The computed outcome for one employee for a given run — age at the reference date, average monthly net salary, eligibility flag, rank among eligible employees, selected flag, the 12 monthly deductible amounts, and their total. Represents one row of the output.
...
- An employee with fewer than 12 months of populated data (e.g., a new hire) has their average monthly net salary computed over only the months that contain data for them, not divided by 12; this is treated as a documented, deliberate choice rather than an open question.
